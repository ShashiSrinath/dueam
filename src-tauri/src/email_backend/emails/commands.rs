...
use crate::utils::attachments::{read_attachment_data, save_attachment_data};
...
#[tauri::command]
pub async fn get_unified_counts<R: tauri::Runtime>(
    app_handle: tauri::AppHandle<R>,
) -> Result<UnifiedCounts, String> {
    let pool = app_handle.state::<SqlitePool>();
...
#[tauri::command]
pub async fn get_email_by_id<R: tauri::Runtime>(
    app_handle: tauri::AppHandle<R>,
    email_id: i64,
) -> Result<Email, String> {
    let pool = app_handle.state::<SqlitePool>();
...
#[tauri::command]
pub async fn get_email_content<R: tauri::Runtime>(
    app_handle: tauri::AppHandle<R>,
    email_id: i64,
) -> Result<EmailContent, String> {
    let pool = app_handle.state::<SqlitePool>().inner().clone();

    let cached_info: Option<(Option<String>, Option<String>, Option<String>, bool, i64)> =
        sqlx::query_as(
    ...
    let existing_summary = cached_info
        .as_ref()
        .and_then(|(_, _, summary, _, _)| summary.clone());
            ...
            if !has_attachments || attachment_count > 0 {
                if summary.is_none() {
                    if let Some(text) = body_text.clone() {
                        trigger_email_summary(&app_handle, &pool, email_id, text);
                    }
                }
    ...
    client
        .examine_mailbox(&folder_path)
        .await
        .map_err(|e| e.to_string())?;

    use std::num::NonZeroU32;
    let uids: imap_client::imap_next::imap_types::sequence::SequenceSet = id
        .iter()
    ...
    let messages = client
        .fetch_messages_with_items(uids, fetch_items)
        .await
        .map_err(|e| e.to_string())?;
...
fn trigger_email_summary<R: tauri::Runtime>(
    app_handle: &tauri::AppHandle<R>,
    pool: &SqlitePool,
    email_id: i64,
    text: String,
    ...
    tauri::async_runtime::spawn(async move {
        let settings: Vec<(String, String)> = sqlx::query_as(
            "SELECT key, value FROM settings WHERE key IN ('aiEnabled', 'aiSummarizationEnabled')",
        )
        .fetch_all(&pool)
        .await
        .unwrap_or_default();
        ...
        let mut summarization_enabled = false;

        for (key, value) in settings {
            let parsed = serde_json::from_str::<bool>(&value).unwrap_or(false);
            match key.as_str() {
                "aiEnabled" => ai_enabled = parsed,
                "aiSummarizationEnabled" => summarization_enabled = parsed,
                _ => {}
            }
        }

        if !ai_enabled || !summarization_enabled {
            return;
        }
        ...
        if let Err(e) = crate::email_backend::llm::summarization::enqueue_summarization(
            ...
            email_id,
            &text,
            crate::email_backend::llm::AiTaskPriority::Normal,
        )
        .await
        {
            log::debug!("Failed to enqueue summarization for email {}: {}", email_id, e);
        }
    });
}
...
pub async fn regenerate_summary<R: tauri::Runtime>(
    ...
    let pool = app_handle.state::<SqlitePool>();
    let controller = app_handle.state::<std::sync::Arc<crate::email_backend::llm::controller::AiController>>();
    ...
    let task_id = crate::email_backend::llm::controller::AiController::task_id_from_summarize(email_id);
    let task = crate::email_backend::llm::controller::AiTask::Summarize {
        email_id,
        body_text: text,
    };

    let result = controller
        .process_task_sync(&app_handle, &pool, task_id, task)
        .await;
...
pub async fn save_draft<R: tauri::Runtime>(
    ...
    for att_id in attachment_ids {
        ...
        let att: Option<(Option<String>, Option<String>, i64, Option<String>)> = sqlx::query_as(
            "SELECT filename, mime_type, size, file_hash FROM attachments WHERE id = ?",
        )
        .bind(att_id)
        .fetch_optional(&*pool)
        .await
...
#[tauri::command]
pub async fn get_drafts<R: tauri::Runtime>(
    app_handle: tauri::AppHandle<R>,
    account_id: i64,
) -> Result<Vec<Draft>, String> {
    let pool = app_handle.state::<SqlitePool>();
    let mut drafts = sqlx::query_as::<_, Draft>(
...
#[tauri::command]
pub async fn get_draft_by_id<R: tauri::Runtime>(
    app_handle: tauri::AppHandle<R>,
    id: i64,
) -> Result<Draft, String> {
    let pool = app_handle.state::<SqlitePool>();
...
#[tauri::command]
pub async fn delete_draft<R: tauri::Runtime>(
    app_handle: tauri::AppHandle<R>,
    id: i64,
) -> Result<(), String> {
    let pool = app_handle.state::<SqlitePool>();
...
#[tauri::command]
pub async fn mark_as_read<R: tauri::Runtime>(
    app_handle: tauri::AppHandle<R>,
    email_ids: Vec<i64>,
) -> Result<(), String> {
    let pool = app_handle.state::<SqlitePool>();
...
#[tauri::command]
pub async fn move_to_inbox<R: tauri::Runtime>(
    app_handle: tauri::AppHandle<R>,
    email_ids: Vec<i64>,
) -> Result<(), String> {
    let pool = app_handle.state::<SqlitePool>();
    ...
    for &email_id in &email_ids {
        ...
        let inbox_folder_info: Option<(i64, String)> =
            sqlx::query_as("SELECT id, path FROM folders WHERE account_id = ? AND role = 'inbox'")
                .bind(account_id)
                .fetch_optional(&*pool)
                .await
                .map_err(|e| e.to_string())?;
        ...
        if let Ok(backend) = engine.get_backend(account_id).await {
            ...
            let _ = backend
                .move_messages(&source_folder_path, &inbox_folder_path, &id)
                .await
                .map_err(|e| e.to_string())?;
        }
        ...
        let is_unread: bool =
            sqlx::query_scalar("SELECT flags NOT LIKE '%seen%' FROM emails WHERE id = ?")
                .bind(email_id)
                .fetch_one(&mut *tx)
                .await
                .map_err(|e| e.to_string())?;
...
#[tauri::command]
pub async fn archive_emails<R: tauri::Runtime>(
    app_handle: tauri::AppHandle<R>,
    email_ids: Vec<i64>,
) -> Result<(), String> {
    let pool = app_handle.state::<SqlitePool>();
    ...
    for &email_id in &email_ids {
        ...
        let (archive_folder_id, archive_folder_path) = match archive_folder_info {
            ...
            None => {
                return Err(format!(
                    "Archive folder not found for account {}",
                    account_id
                ))
            }
        };
        ...
        if let Ok(backend) = engine.get_backend(account_id).await {
            ...
            use email::message::r#move::MoveMessages;
            let _ = backend
                .move_messages(&source_folder_path, &archive_folder_path, &id)
                .await
                .map_err(|e| e.to_string())?;
        }
...
#[tauri::command]
pub async fn move_to_trash<R: tauri::Runtime>(
    app_handle: tauri::AppHandle<R>,
    email_ids: Vec<i64>,
) -> Result<(), String> {
    let pool = app_handle.state::<SqlitePool>();
    ...
    for &email_id in &email_ids {
        ...
        let trash_folder_info: Option<(i64, String)> =
            sqlx::query_as("SELECT id, path FROM folders WHERE account_id = ? AND role = 'trash'")
                .bind(account_id)
                .fetch_optional(&*pool)
                .await
                .map_err(|e| e.to_string())?;
        ...
        if let Ok(backend) = engine.get_backend(account_id).await {
            ...
            use email::message::r#move::MoveMessages;
            let _ = backend
                .move_messages(&source_folder_path, &trash_folder_path, &id)
                .await
                .map_err(|e| e.to_string())?;
        }
...
#[tauri::command]
pub async fn get_attachments<R: tauri::Runtime>(
    app_handle: tauri::AppHandle<R>,
    email_id: i64,
...
async fn fetch_attachment_data_internal<R: tauri::Runtime>(
    app_handle: &tauri::AppHandle<R>,
    attachment_id: i64,
...
#[tauri::command]
pub async fn get_attachment_data<R: tauri::Runtime>(
    app_handle: tauri::AppHandle<R>,
    attachment_id: i64,
) -> Result<Vec<u8>, String> {
...
#[tauri::command]
pub async fn save_attachment_to_path<R: tauri::Runtime>(
    app_handle: tauri::AppHandle<R>,
    attachment_id: i64,
    path: String,
) -> Result<(), String> {
...
#[tauri::command]
pub async fn open_attachment<R: tauri::Runtime>(
    app_handle: tauri::AppHandle<R>,
    attachment_id: i64,
) -> Result<(), String> {
    let pool = app_handle.state::<SqlitePool>().inner().clone();
    ...
    // 2. Get filename
    let filename: String =
        sqlx::query_scalar::<_, Option<String>>("SELECT filename FROM attachments WHERE id = ?")
            .bind(attachment_id)
            .fetch_one(&pool)
            .await
            .map_err(|e| e.to_string())?
...
pub async fn send_email<R: tauri::Runtime>(
    ...
    for id in attachment_ids {
        let att_info: (Option<String>, Option<String>) =
            sqlx::query_as("SELECT filename, mime_type FROM attachments WHERE id = ?")
                .bind(id)
                .fetch_one(&*pool)
                .await
                .map_err(|e| e.to_string())?;
    ...
    let backend = match backend_builder.build().await {
        ...
        Err(e) => {
            ...
            {
                info!(
                    "Refreshing token for account {} due to build error: {}",
                    account.email(),
                    err_str
                );
    ...
    if let Err(e) = backend.send_message(&message).await {
        ...
        {
            info!(
                "Refreshing token for account {} due to send error: {}",
                account.email(),
                err_str
            );
            manager.refresh_access_token(account.email()).await?;
    ...
    let sent_folder: Option<(i64, String)> =
        sqlx::query_as("SELECT id, path FROM folders WHERE account_id = ? AND role = 'sent'")
            .bind(account_id)
            .fetch_optional(&*pool)
            .await
            .map_err(|e| e.to_string())?;
    ...
    if let Some((folder_id, path)) = sent_folder {
        if let Ok(backend) = engine.get_backend(account_id).await {
            ...
            let _ = backend
                .add_message_with_flags(&path, &message, &flags)
                .await;
    ...
    let flat_recipients: Vec<String> = all_recipients
        .iter()
        .flat_map(|s| s.split(',').map(|p| p.trim().to_string()))
        .filter(|s| !s.is_empty())
        .collect();

    let _ = crate::email_backend::enrichment::commands::save_recipients_as_contacts(
        &app_handle,
        flat_recipients,
    )
    .await;
...
#[tauri::command]
pub async fn get_folders<R: tauri::Runtime>(
    app_handle: tauri::AppHandle<R>,
    account_id: i64,
