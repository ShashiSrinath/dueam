    ...
    R: BackendContextBuilder + SyncHash + 'static,
{
    ...
    pub async fn sync(self) -> Result<SyncReport> {
        ...
        left_lock_file
            .try_lock()
            .map_err(|err| Error::LockFileError(advisory_lock::FileLockError::Io(err.into()), left_lock_file_path.clone()))?;
        ...
        right_lock_file
            .try_lock()
            .map_err(|err| Error::LockFileError(advisory_lock::FileLockError::Io(err.into()), right_lock_file_path.clone()))?;
        ...
        left_lock_file
            .unlock()
            .map_err(|err| Error::UnlockFileError(advisory_lock::FileLockError::Io(err), left_lock_file_path))?;
        right_lock_file
            .unlock()
            .map_err(|err| Error::UnlockFileError(advisory_lock::FileLockError::Io(err), right_lock_file_path))?;
